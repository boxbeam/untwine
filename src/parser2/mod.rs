...
macro_rules! chain_kind {
    ($a:ident , $b:ident) => {
        <ChainImpl<$a::Kind, $b::Kind> as Chain>::NextKind
    };
    ($a:ident , $b:ident , $($rest:ident),+) => {
        <ChainImpl<$a::Kind, chain_kind!($b , $($rest),+)> as Chain>::NextKind
    };
}

macro_rules! chain_typ {
    ($a:ident , $b:ident , $($rest:ident),+) => {
        <ChainImpl<$a::Kind, chain_kind!($b , $($rest),+)> as Chain>::Output<$a::Output<'a>, chain_typ!($b , $($rest),+)>
    };
    ($a:ident , $b:ident) => {
        <ChainImpl<$a::Kind, $b::Kind> as Chain>::Output<$a::Output<'a>, $b::Output<'a>>
    };
}

macro_rules! chain_val {
    ($input:ident, $offset:ident, $errs:ident, $ctx:ident | $parser_a:ident : $var_a:ident, $parser_b:ident : $var_b:ident) => {
        <ChainImpl<$parser_a::Kind, $parser_b::Kind> as Chain>::chain(
            {
                let (len, e) = $var_a.parse($input.skip($offset), $errs.clone(), $ctx)?;
                $offset += len;
                e
            },
            {
                let (len, e) = $var_b.parse($input.skip($offset), $errs.clone(), $ctx)?;
                $offset += len;
                e
            },
        )
    };
    ($input:ident, $offset:ident, $errs:ident, $ctx:ident | $parser_a:ident : $var_a:ident, $parser_b:ident : $var_b:ident , $( $parser_rest:ident : $var_rest:ident ),+) => {
        <ChainImpl<$parser_a::Kind, $parser_b::Kind> as Chain>::chain(
            {
                let (len, e) = $var_a.parse($input.skip($offset), $errs.clone(), $ctx)?;
                $offset += len;
            ...
            chain_val!($input, $offset, $errs, $ctx | $parser_b : $var_b , $( $parser_rest : $var_rest ),+),
        )
    };
}

macro_rules! impl_tuple_parse {
    ($($parser:ident : $name:ident),+) => {
        fn parse<'a>(
            &mut self,
            input: Input<'a>,
            errs: impl ErrorHandler<E>,
            ...
            let mut offset = 0;
            let ($($name),+) = self;
            let val = chain_val!(input, offset, errs, ctx | $($parser: $name),+);
            Some((offset, val))
        }
...
impl<P1, P2, E, C> Parser<E, C> for (P1, P2)
where
    P1: Parser<E, C>,
    P2: Parser<E, C>,
    ChainImpl<P1::Kind, P2::Kind>: Chain,
{
    type Output<'a> = chain_typ!(P1, P2);
    type Kind = chain_kind!(P1, P2);

    impl_tuple_parse!(P1: a, P2: b);
}

impl<P1, P2, P3, E, C> Parser<E, C> for (P1, P2, P3)
where
    P1: Parser<E, C>,
    P2: Parser<E, C>,
    P3: Parser<E, C>,
    ChainImpl<P1::Kind, P2::Kind>: Chain,
    ChainImpl<P2::Kind, P3::Kind>: Chain,
    ChainImpl<P1::Kind, <ChainImpl<P2::Kind, P3::Kind> as Chain>::NextKind>: Chain,
{
    type Output<'a> = chain_typ!(P1, P2, P3);
    type Kind = chain_kind!(P1, P2, P3);

    impl_tuple_parse!(P1: a, P2: b, P3: c);
}

macro_rules! impl_parser_tuple {
    ($($parser:ident : $name:ident),+) => {
        impl<$($parser),+, E, C> Parser<E, C> for ($($parser),+)
        where
            P1: Parser<E, C>,
            P2: Parser<E, C>,
            ChainImpl<P1::Kind, P2::Kind>: Chain,
        {
            ...
            fn parse<'a>(
                ...
                let mut offset = 0;
                let (a, b) = self;
                let val = chain_val!(input, offset, errs, ctx | P1: a, P2: b);
                Some((offset, val))
            }
        }
    }
